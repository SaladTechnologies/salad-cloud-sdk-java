...
public class LoggingSplunk2Validator extends AbstractModelValidator<LoggingSplunk2> {
  ...
  protected Violation[] validateModel(LoggingSplunk2 loggingSplunk2) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingSplunk2.getHost()))
      .add(new StringValidator("token").minLength(1).maxLength(1000).required().validate(loggingSplunk2.getToken()))
      .aggregate();
  }
}
