...
public class LoggingTcp2Validator extends AbstractModelValidator<LoggingTcp2> {
  ...
  protected Violation[] validateModel(LoggingTcp2 loggingTcp2) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingTcp2.getHost()))
      .add(new NumericValidator<Long>("port").min(1L).max(65535L).required().validate(loggingTcp2.getPort()))
      .aggregate();
  }
}
