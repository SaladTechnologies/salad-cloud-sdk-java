...
public class LoggingSplunk3Validator extends AbstractModelValidator<LoggingSplunk3> {
  ...
  protected Violation[] validateModel(LoggingSplunk3 loggingSplunk3) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingSplunk3.getHost()))
      .add(new StringValidator("token").minLength(1).maxLength(1000).required().validate(loggingSplunk3.getToken()))
      .aggregate();
  }
}
