...
public class LoggingHttp2Validator extends AbstractModelValidator<LoggingHttp2> {
  ...
  protected Violation[] validateModel(LoggingHttp2 loggingHttp2) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingHttp2.getHost()))
      .add(new NumericValidator<Long>("port").min(1L).max(65535L).required().validate(loggingHttp2.getPort()))
      .add(new ListValidator<HttpHeaders3>("headers").maxLength(1000).optional().validate(loggingHttp2.getHeaders()))
      .aggregate();
  }
}
