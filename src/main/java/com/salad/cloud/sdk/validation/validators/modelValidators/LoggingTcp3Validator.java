...
public class LoggingTcp3Validator extends AbstractModelValidator<LoggingTcp3> {
  ...
  protected Violation[] validateModel(LoggingTcp3 loggingTcp3) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingTcp3.getHost()))
      .add(new NumericValidator<Long>("port").min(1L).max(65535L).required().validate(loggingTcp3.getPort()))
      .aggregate();
  }
}
