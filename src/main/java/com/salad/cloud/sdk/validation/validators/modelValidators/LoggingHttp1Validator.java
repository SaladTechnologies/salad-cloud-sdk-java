...
public class LoggingHttp1Validator extends AbstractModelValidator<LoggingHttp1> {
  ...
  protected Violation[] validateModel(LoggingHttp1 loggingHttp1) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingHttp1.getHost()))
      .add(new NumericValidator<Long>("port").min(1L).max(65535L).required().validate(loggingHttp1.getPort()))
      .add(new ListValidator<HttpHeaders1>("headers").maxLength(1000).optional().validate(loggingHttp1.getHeaders()))
      .aggregate();
  }
}
