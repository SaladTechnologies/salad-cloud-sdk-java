...
public class LoggingSplunk1Validator extends AbstractModelValidator<LoggingSplunk1> {
  ...
  protected Violation[] validateModel(LoggingSplunk1 loggingSplunk1) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingSplunk1.getHost()))
      .add(new StringValidator("token").minLength(1).maxLength(1000).required().validate(loggingSplunk1.getToken()))
      .aggregate();
  }
}
