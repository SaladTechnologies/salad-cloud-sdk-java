...
public class LoggingTcp1Validator extends AbstractModelValidator<LoggingTcp1> {
  ...
  protected Violation[] validateModel(LoggingTcp1 loggingTcp1) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingTcp1.getHost()))
      .add(new NumericValidator<Long>("port").min(1L).max(65535L).required().validate(loggingTcp1.getPort()))
      .aggregate();
  }
}
