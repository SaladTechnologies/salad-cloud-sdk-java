...
public class LoggingDatadog2Validator extends AbstractModelValidator<LoggingDatadog2> {
  ...
  protected Violation[] validateModel(LoggingDatadog2 loggingDatadog2) {
    return new ViolationAggregator()
      .add(new StringValidator("host").minLength(1).maxLength(1000).required().validate(loggingDatadog2.getHost()))
      .add(new StringValidator("apiKey").minLength(1).maxLength(1000).required().validate(loggingDatadog2.getApiKey()))
      .add(new ListValidator<DatadogTags2>("tags").maxLength(1000).optional().validate(loggingDatadog2.getTags()))
      .aggregate();
  }
}
